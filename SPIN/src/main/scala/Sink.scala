...
case class StartSPIN()
case class StopSPIN()
case class IncrementDataMessageCounter()
case class IncrementControlMessageCounter()
case class NodeReceivedData(index: Int)
...
class Sink(acsys: ActorSystem, numNodes: Int, topology: String) extends Actor{
	...
	var CONTROLMESSAGESCOUNT =0
	var DATAMESSAGESCOUNT = 0
	...
	def receive ={
		...
		case StartSPIN()=>
			...
			println("\n"+"Start sensing data!! " + r +"th node in the ActorsList!" +"\n")
			ActorsList(r) ! SenseData()	


		case NodeReceivedData(index:Int) =>
			Nodes_WithUpdatedData(index) =1
			//println("Did all nodes receive updated data(1 represents yes)??: "+ Nodes_WithUpdatedData)
			if (!(Nodes_WithUpdatedData.contains(0))){
				...
				println("\n-----Total number of control messages passed: "+ CONTROLMESSAGESCOUNT)
				println("\n-----Total number of data messages passed: "+ DATAMESSAGESCOUNT)
				val end_time: Long = System.currentTimeMillis
				val time_taken: Long = end_time -start_time
				println("\n------Total time taken: "+ time_taken)
				var totalData = DATAMESSAGESCOUNT*1000 + CONTROLMESSAGESCOUNT
				println("\n-----Bandwidth required: " + ((totalData.toDouble)/time_taken)+ "\n\n")
		...
		case IncrementControlMessageCounter() =>
			CONTROLMESSAGESCOUNT += 1

		case IncrementDataMessageCounter() =>
			DATAMESSAGESCOUNT += 1
